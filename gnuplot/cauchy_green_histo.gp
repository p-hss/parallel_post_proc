...
if(mhd == 1){
    ...
    set output "figures/histograms/mhd_cg_histo_20_256.tex"

    f(x) = x>0.00000001 ? x : 1/0
    set style fill transparent solid 1 noborder
    set style circle radius 0.001

    plot '<sort -g -k3 ../data/sim_M04/cg_histo_'.sim.'_tkolmo_20.dat' u 1:2:(0.10):(f($3))\
...
if(mhd == 0){
    ...
    set output "figures/histograms/hd_cg_histo_20_256.tex"

    f(x) = x>0.0001 ? x : 1/0
    set style fill transparent solid 1 noborder
    set style circle radius 0.001
    
    plot '<sort -g -k3 ../data/sim_H00/cg_histo_'.sim.'_tkolmo_20.dat' u 1:2:(0.10):(f($3))\
    w circles lc palette
    #title '', p(x) w l title "Pancake shape" ls 50, c(x) title "Cigar shape" ls 51
